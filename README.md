...
- **거래량 Y축**: 주기 인지 포맷 — 일/주봉은 K 고정(예: `1000K` = 100만주), 5분봉은 raw 콤마(`1,500`)
  - finplot `YAxisItem` 클래스 패치 + `vb.yscale.xform` 으로 정규화값을 실제 거래량으로 환산해 표시
- **단위 라벨** Y축에 세로 표시: "거래량 (주)", "(억원)" 등
- **X축 시간 라벨**: 줌 레벨에 따라 자동 단계 — 7일↑ 날짜만, 3~7일 6h, 1~3일 3h, 9h~1일 2h, 3~9h 1h
- **일목균형표** 토글: 선행스팬1·2를 빨강/파랑 점선 표시 (HTS 표준 9/26/52/26 파라미터, **거래일 기준 26봉 forward**)
  - 토글 시 차트 재렌더 없이 senkou plot 아이템만 add/remove → **줌·스크롤 100% 보존**
- **지수 비교 오버레이**: 코스피·코스피 200·KRX 300·KRX 100·코스닥·코스닥 150 정규화 라인
  - 진한 주황(#ff8800) 1.5px + finplot legend + 라인 끝점에 `"코스피 (정규화)"` 텍스트 라벨 → 캡쳐만 봐도 라인 정체 식별
  - 첫 fetch 시 status 진행 표시(`코스피 지수 불러오는 중 (12/250)…`) + processEvents 로 UI 응답성 유지
...
### 💰 수급 탭
- 외국인·기관·개인 **누적 순매수** (~6개월/120 거래일치, Naver frgn 스크래핑)
- 캐시 재사용 판정을 직전 영업일 기준으로 변경 — 평일 오래된 캐시(예: 4/29) 그대로 반환되던 문제 해결
...
### 📕 스텔스 모드 (회사용 🤫)
- **앱 시작 시 기본 ON** — 켜자마자 회사 화면에 안전한 모드
- **단축키 `A`** 로 즉시 토글 (QApplication eventFilter — QLineEdit 포커스 중에는 무시되어 코드 입력 방해 X)
- 토글 시 차트·탭·종목코드 입력란·버튼 모두 숨김 (입력란 숨겨서 포커스 가로채는 문제 방지)
- **종목명 + 가격 + 등락률만** 작은 흑색 글씨로 표시
...
기본 종목 한화비전(489790). 첫 화면은 **스텔스 모드**(작은 흑색 라벨)로 뜸 → `A` 키 또는 📖 버튼으로 일반 모드 진입 → 상단 입력란에 6자리 종목코드 입력 후 Enter 또는 [불러오기].
